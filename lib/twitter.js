...
Twitter.prototype.getShortLink = function (data) {
  ...
  var tweetURL = 'http://twitter.com/#!/'
                 ...
                 + data.id_str,
      self = this;
  return self.bitly.shorten(tweetURL, function (err, result) {
    if (err) {return self.emit('error::getShortLink', err.message);}
    // Check result.status_code here for bit.ly API errors
    if (result.status_code !== 200) {
      return self.emit('error::getShortLink', result.status_txt);
      }
    if (result.data.url) {
...
Twitter.prototype.followUser = function (id) {
  var self = this;
